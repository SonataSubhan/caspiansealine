...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    getContent(locale).ports.map((entry) => ({ locale, slug: entry.slug }))
  );
}
...
export default async function Image({ params }) {
  ...
  const entry = bySlug(content.ports, slug);

  return ogCard({
    eyebrow: content.ui.card.port,
    title: entry?.name ?? content.site.tagline,
  });
}
